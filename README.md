...
一个安全且隐私优先的邮件搜索系统，允许用户在不向服务器泄露邮件内容或搜索关键词的前提下，完成邮件的搜索与解密。本系统基于 Flask、Jieba（中文 NLP）和现代 Web 加密技术构建。

 (建议替换为实际的系统架构图：客户端构建索引 -> 服务器存储密文 -> 客户端本地解密)
...
│   ├── private_key.pem         # 用户私钥（⚠️ 仅在客户端保留！）
...
安装依赖
git clone https://github.com/your-username/privacy-email-search.git
cd privacy-email-search
pip install -r requirements.txt

数据准备与索引构建
将原始邮件文本文件（.txt）放入 data/ham/ 目录。
关键步骤：运行客户端工具构建加密索引。
      python client_se_index_builder.py
   
   > ⚠️ 注意：此步骤会生成 private_key.pem（请务必妥善保存！）并将加密后的索引上传至服务器。服务器仅存储哈希值，无法反推关键词。
...
使用流程
加载私钥：在网页顶部上传刚才生成的 private_key.pem 文件（私钥仅存于浏览器内存）。
搜索邮件：输入中文关键词（如“发票”、“合同”、“会议”）。
解密查看：点击搜索结果中的 "🔓 解密查看" 按钮。
隐私验证：观察控制台和网络请求，确认解密密钥和内容从未离开过您的浏览器。
